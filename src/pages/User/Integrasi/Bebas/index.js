...
const Bebas = ({ navigation }) => {
  ...
  const fetchAnnouncements = async () => {
    try {
      const announcements = await pb.collection("wbp_bebas").getFullList();
  ...
  return (
    <View style={styles.container}>
      <View style={styles.content}>
        <Text style={styles.title}>Bebas</Text>
        <ScrollView>
          {announcements.map((announcement) => (
            <View key={announcement.id} style={styles.announcementCard}>
              <Text style={styles.contentText}>{announcement.bebas}</Text>
            </View>
          ))}
          ...
          {announcements.map((announcement) => (
            <TouchableOpacity
              key={announcement.id}
              onPress={() =>
                handleFileClick("wbp_bebas", announcement.id, announcement.bebas_gambar)
              }
              style={styles.fileContainer}
            >
              <Ionicons name="document-outline" size={24} color={colors.primary} />
              <Text style={styles.contentText}>{announcement.bebas_gambar}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      </View>
